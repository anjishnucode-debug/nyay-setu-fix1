...
function getCustomPrisoners() {
  if (!isBrowser) return [];
  try {
    const data = localStorage.getItem('custom_prisoners');
...
export let prisoners = [...defaultPrisoners, ...getCustomPrisoners()];

export function addPrisoner(p) {
  if (isBrowser) {
    try {
      const customs = getCustomPrisoners();
      customs.push(p);
      localStorage.setItem('custom_prisoners', JSON.stringify(customs));
    } catch (e) {
      console.error('Error saving prisoner:', e);
    }
  }
  prisoners.push(p);
}
...
export function searchPrisoners(query) {
  ...
  return prisoners.filter(p =>
    ...
    (p.prisoner_name || p.name || '').toLowerCase().includes(q) ||
