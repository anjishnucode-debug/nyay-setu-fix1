...
export default function NyayMitra() {
  ...
  async function processFile(file) {
    ...
    setFileName(file.name); setError(''); setStage('processing'); setAnalyzing(true);
    try {
      const data = await aiAPI.analyzePDF(file);
      if (data.success) { setResult(data.analysis); setStage('result'); }
      else throw new Error('Analysis failed. Please try again.');
    } catch (err) {
      setError(err.message || 'Upload failed.'); setStage('upload');
    } finally { setAnalyzing(false); }
  }

  async function loadSampleCase() {
    setFileName('Sample_Case_SC_BLR_2023_4521.pdf');
    setError(''); setStage('processing'); setAnalyzing(true);
    try {
      const data = await aiAPI.analyzeText(SAMPLE_CASE_TEXT);
      if (data.success) { setResult(data.analysis); setStage('result'); }
      else throw new Error('Sample analysis failed.');
    } catch (err) {
      setError(err.message || 'Sample analysis failed.'); setStage('upload');
    } finally { setAnalyzing(false); }
  }
  ...
  function handleReset() { setStage('upload'); setResult(null); setError(''); setFileName(''); }
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start',
          ...
          <div>
            <div style={{ fontSize: '0.72rem', letterSpacing: 2, textTransform: 'uppercase',
              color: 'var(--gold-dim)', marginBottom: '0.4rem' }}>AI Case Intelligence</div>
            <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '2.2rem',
              fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.3rem' }}>Nyay Mitra</h1>
          ...
          {stage === 'result' && (
            <button onClick={handleReset}
              style={{ padding: '0.6rem 1.2rem', background: 'transparent',
                ...
                color: 'var(--gold-dim)', cursor: 'pointer', fontSize: '0.8rem',
        ...
        {stage === 'upload' && (
          <div>
            ...
            <div
              ...
              style={{ border: `2px dashed ${dragOver ? 'var(--gold)' : 'var(--border-gold)'}`,
                borderRadius: 16, padding: '3.5rem 2rem', textAlign: 'center',
                cursor: 'pointer', transition: 'all 0.25s', marginBottom: '1.2rem',
                background: dragOver ? 'var(--gold-faint)' : 'var(--bg-card)' }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--gold-faint)'}
              onMouseLeave={e => e.currentTarget.style.background = dragOver ? 'var(--gold-faint)' : 'var(--bg-card)'}>
              <input ref={fileRef} type="file" accept=".pdf" style={{ display: 'none' }}
                onChange={e => e.target.files[0] && processFile(e.target.files[0])} />
              <div style={{ fontSize: '2.5rem', marginBottom: '0.8rem' }}>📄</div>
              <div style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 500, marginBottom: '0.4rem' }}>
                Drop case PDF here
              </div>
              <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                or click to browse — chargesheets, FIRs, court orders
              </div>
              <div style={{ display: 'inline-block', padding: '0.6rem 1.5rem',
                background: 'var(--gold-faint)', border: '1px solid var(--border-gold)',
                borderRadius: 8, fontSize: '0.82rem', color: 'var(--gold)' }}>
                Browse PDF →
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '1rem' }}>
              <button onClick={loadSampleCase}
                ...
                onMouseEnter={e => { e.target.style.background = 'var(--gold-faint)'; e.target.style.color = 'var(--gold)'; }}
                onMouseLeave={e => { e.target.style.background = 'transparent'; e.target.style.color = 'var(--gold-dim)'; }}>
        ...
        {stage === 'processing' && (
          <div style={{ ...cardStyle, padding: '4rem 2rem', textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.4rem',
              ...
              {analyzing ? 'Analyzing with AI...' : 'Extracting text from PDF...'}
            </div>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{fileName}</div>
            <div style={{ marginTop: '1.5rem', fontSize: '0.75rem', color: 'var(--gold-dim)' }}>
              Nyay Mitra is preparing your case brief
            </div>
          </div>
        )}
        ...
        {stage === 'result' && result && (
          <div>
            <div style={{ ...cardStyle, padding: '1.5rem 1.8rem', marginBottom: '1rem',
              background: 'var(--gold-faint)', border: '1px solid var(--border-gold-hover)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ fontSize: '0.65rem', letterSpacing: 2, textTransform: 'uppercase',
                    color: 'var(--gold-dim)', marginBottom: '0.4rem' }}>Case Analysis Complete</div>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.6rem',
                    fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.3rem' }}>{result.case_title}</div>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '0.65rem', letterSpacing: 1, textTransform: 'uppercase',
                    color: 'var(--gold-dim)', marginBottom: 4 }}>Next Hearing</div>
                  <div style={{ fontSize: '1rem', fontWeight: 500, color: 'var(--gold)' }}>
                    {result.next_hearing}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '1.2rem' }}>
                {[
                  ...
                  <div key={i} style={{ padding: '4px 12px', background: 'var(--bg-card)',
                    border: '1px solid var(--border-gold)', borderRadius: 20 }}>
                    <span style={{ fontSize: '0.65rem', color: 'var(--gold-dim)',
                      textTransform: 'uppercase', letterSpacing: 1 }}>{chip.label}: </span>
                    <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>{chip.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr',
              ...
              <div style={{ ...cardStyle, padding: '1.2rem 1.4rem' }}>
                <div style={{ fontSize: '0.65rem', letterSpacing: 2, textTransform: 'uppercase',
                  color: 'var(--gold-dim)', marginBottom: '0.8rem' }}>Accused</div>
                {result.accused?.map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start',
                    ...
                    <div style={{ width: 20, height: 20, borderRadius: '50%',
                      ...
                      fontSize: '0.65rem', color: 'var(--gold)', flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
                    <span style={{ fontSize: '0.82rem', color: 'var(--text-primary)', lineHeight: 1.4 }}>{a}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ ...cardStyle, padding: '1.2rem 1.4rem' }}>
                <div style={{ fontSize: '0.65rem', letterSpacing: 2, textTransform: 'uppercase',
                  color: 'var(--gold-dim)', marginBottom: '0.8rem' }}>Key Facts</div>
                {result.key_facts?.map((f, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, marginBottom: '0.5rem', alignItems: 'flex-start' }}>
                    ...
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{f}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {result.important_dates?.length > 0 && (
              <div style={{ ...cardStyle, padding: '1.4rem 1.8rem', marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.65rem', letterSpacing: 2, textTransform: 'uppercase',
                  color: 'var(--gold-dim)', marginBottom: '1rem' }}>Case Timeline</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0' }}>
                  {result.important_dates.map((d, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem' }}>
                      <div style={{ padding: '0.4rem 0.9rem', background: 'var(--gold-faint)',
                        border: '1px solid var(--border-gold)', borderRadius: 6 }}>
                        <div style={{ fontSize: '0.65rem', color: 'var(--gold-dim)', marginBottom: 1 }}>{d.event}</div>
                        <div style={{ fontSize: '0.78rem', color: 'var(--text-primary)', fontWeight: 500 }}>{d.date}</div>
                      </div>
                      {i < result.important_dates.length - 1 && (
                        <div style={{ width: 20, height: 1, background: 'var(--border-gold)', margin: '0 4px' }} />
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div style={{ ...cardStyle, padding: '1.4rem 1.8rem',
              background: 'var(--gold-faint)', border: '1px solid var(--border-gold)' }}>
              <div style={{ fontSize: '0.65rem', letterSpacing: 2, textTransform: 'uppercase',
                color: 'var(--gold-dim)', marginBottom: '0.7rem' }}>AI Case Summary</div>
